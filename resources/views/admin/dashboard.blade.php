...
function setMetricValue(id, value) {
    ...
    if (element) {
        element.textContent = Number(value || 0).toLocaleString('es-CO');
    }
}
...
function actualizarUsuariosActivos() {
    ...
    fetch('{{ route('api.admin.usuarios-activos') }}')
        ...
        .then(usuarios => {
                ...
                usuarios.forEach(usuario => {
                    ...
                    html += `
                        <tr class="hover:bg-blue-50/70 cursor-pointer transition-colors"
                            ...
                            <td class="py-3 px-4 whitespace-nowrap">
                                <span class="dashboard-badge px-2 py-1 rounded-md text-xs font-medium ${rolClass}">
                                    ${usuario.rol}
                                </span>
                            </td>
                            <td class="py-3 px-4 whitespace-nowrap text-sm text-gray-900">
                            ...
                            <td class="py-3 px-4 whitespace-nowrap">
                                <span class="dashboard-badge px-2 py-1 rounded-md text-xs font-medium ${statusClass}">
                                    ${usuario.status}
                                </span>
                            </td>
                            <td class="py-3 px-4 text-sm text-gray-900">
            ...
            setMetricValue('metric-usuarios-activos', usuarios.length);
            setMetricValue('metric-usuarios-disponibles', usuarios.filter(usuario => usuario.status === 'DISPONIBLE').length);
            setMetricValue('metric-usuarios-ocupados', usuarios.filter(usuario => usuario.status === 'OCUPADO').length);
...
function actualizarTurnosPorServicio() {
    ...
    fetch('{{ route('api.admin.turnos-por-servicio') }}')
        ...
        .then(turnos => {
            ...
            setMetricValue('metric-turnos-atendidos', turnos.reduce((total, turno) => total + Number(turno.terminados || 0), 0));
...
function actualizarTurnosPorAsesor() {
    ...
    fetch('{{ route('api.admin.turnos-por-asesor') }}')
        ...
        .then(turnos => {
            ...
            setMetricValue('metric-asesores-atencion', turnos.length);
...
function actualizarTurnosEnCola() {
    ...
    fetch('{{ route('api.admin.turnos-en-cola') }}')
        ...
        .then(turnos => {
            ...
            setMetricValue('metric-turnos-cola', turnos.reduce((total, turno) => total + Number(turno.en_cola || 0), 0));
            setMetricValue('metric-servicios-cola', turnos.filter(turno => Number(turno.en_cola || 0) > 0).length);
